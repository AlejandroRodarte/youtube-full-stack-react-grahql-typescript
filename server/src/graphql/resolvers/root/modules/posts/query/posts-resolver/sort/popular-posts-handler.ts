...
const popularPostsHandler = async (args: GraphQLArgs.Args<PostsInput>, db: Connection): Promise<PostsResponse> => {
  ...
  // utils & constants
  const sortMapper = constants.resolvers.root.modules.posts.sortMapper.popular
  ...
  if (data.cursor) {
    ...
    const [createdAt, points] = sortMapper.cursorParser(data.cursor)
            ...
            .where(
              (qb) => {
                ...
                return `
                  ...
                  (
                    (${field} > :points) OR
                    (${field} >= :points AND ${createdAtField} > :createdAt)
                  )
                `
              },
              { createdAt, points }
          ...
          await baseQuery
            .where(
              `
                ...
                (${field} <= :points AND ${createdAtField} < :createdAt)
              `,
              { createdAt: createdAtCursor, points: pointsCursor }
            )
            .take(chunkSize)
            .getRawMany()
...
export default popularPostsHandler
