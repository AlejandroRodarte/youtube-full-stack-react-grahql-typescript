...
export default class UserResolver {
  ...
  async register (
    ...
    try {
      ...
      if (e instanceof UniqueConstraintViolationException && e.code === '23505') return new UsersClasses.responses.RegisterUserResponse(400, 'That username is already taken.', 'USERNAME_EXISTS_ERROR')
      return new UsersClasses.responses.RegisterUserResponse(400, 'There was an error registering the user.', 'MUTATION_REGISTER_ERROR')
    }
  }
  ...
  async login (
    ...
    try {
      ...
      if (!user) return new UsersClasses.responses.LoginUserResponse(404, 'The user was not found.', 'USER_NOT_FOUND_ERROR')
      const isCorrectPassword = await argon2.verify(user.password, data.password)
      if (!isCorrectPassword) return new UsersClasses.responses.LoginUserResponse(401, 'The password is incorrect.', 'USER_PASSWORD_INCORRECT_ERROR')
      req.session.userId = user.id
      ...
      return new UsersClasses.responses.LoginUserResponse(401, 'There was an error logging in the user.', 'MUTATION_LOGIN_ERROR')
    }
  }
}
